...
exports.init= ->
  for modname,cfg of global.conf.modules
    obj=require './request_handlers/'+modname
    if not obj.handle?
      throw new Error('Request handler '+modname+' has no handle() method')
    handlers[obj.request_pathname ? modname]=obj

  if global.conf.default_request?
    default_handler=exports.get global.conf.default_request
